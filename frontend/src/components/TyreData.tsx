...
const OPTIMAL_TEMP = { min: 80, max: 110 };
...
const Tyre: React.FC<TyreProps> = ({ position, surfaceTemp, pressure, wear, damage }) => {
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div className="mt-2 text-xs text-center space-y-1">
        <div className="text-white font-mono">{formatTemperature(surfaceTemp)}</div>
        <div className="text-gray-400 font-mono text-[10px]">{formatPressure(pressure)}</div>
        <div className="text-gray-500 text-[10px]">{wearPercent.toFixed(0)}% wear</div>
      </div>
    </div>
  );
};
